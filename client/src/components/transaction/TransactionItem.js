...
const TransactionItem = ({
  ...
  return(
    <div className="sub-trans bg-white border">
      ...
      <div className="m-2 center">
        <button class="btn btn-warning" onClick={() => { if (window.confirm('Voulez-vous archiver cette commande?')) ;axios.delete(`/api/transactionEnCours/${_id}`); window.location.reload(false);}}>Archiver</button>
      </div>
    </div>
    )};
