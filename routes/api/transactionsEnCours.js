...
const Transaction = require('../../models/TransactionsEnCours');
var bodyParser = require('body-parser');
const TransactionsEnCours = require('../../models/TransactionsEnCours');
...
router.get('/', async(req, res) => {
    try {
        const transaction = await TransactionsEnCours.find({ 'lat' : { $exists: false } }).lean().sort({ date: -1 });
...
router.delete('/:id', async(req, res) => {
    try {
        const trans = await TransactionsEnCours.findById(req.params.id);
        ...
        res.json({ msg: 'Commande archivée' });
    } catch (err) {
    ...
    async(req, res) => {
        ...
        if(total>0){
            ...
            for (var i = 0; i < count; i++) {
                ...
                if(amount>0){
                    try {
                        const transaction = await TransactionsEnCours.findOne({ _id: transaction_id });
...
router.get('/:id', async(req, res) => {
    try {
        const transaction = await TransactionsEnCours.findOne({_id:req.params.id});
